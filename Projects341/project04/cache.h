...
class Cache{
    ...
    ******************************************/
    bool isValidPerson(Person); // checks if person id is valid
    bool personExists(Person); // checks if person exists in cache
    bool isCollision(int); // checks if there is an element at passed index
    int probe(int,string); // probes index in case of collision
    void tryDeleteOld(); // deletes old table if it is empty
    bool keepMoving(); // helper for rehash()
    void rehash(); // incrementally rehashes current table
    void reinsert(Person); // helper for rehash()
};
#endif